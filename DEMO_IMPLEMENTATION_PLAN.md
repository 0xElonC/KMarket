...
sequenceDiagram
    ...
    Note over User, Backend: 阶段 1: 充值
    User->>Frontend: 点击充值 100 USDC
    Frontend->>Contract: 调用 deposit(100)
    Contract-->>EventListener: Emit DepositEvent
    EventListener->>Backend: 监听到充值
    Backend->>Backend: DB: User Balance + 100

    Note over User, Backend: 阶段 2: 高频下注
    User->>Frontend: 选择区间，点击下注
    Frontend->>Frontend: 弹出签名 (EIP-712)
    User->>Frontend: 确认签名 (No Gas)
    Frontend->>Backend: 发送签名 + 下注数据
    Backend->>Backend: 验签 & 扣款 & 记录订单
    loop 每秒检测
        Backend->>Backend: 获取最新价格 -> 判定输赢 -> 加币
    end

    Note over User, Backend: 阶段 3: 提现
    User->>Frontend: 请求提现
    Frontend->>Backend: API: Get Withdraw Signature
    Backend->>Backend: 验证余额 -> 生成私钥签名
    Backend-->>Frontend: 返回 {v, r, s, amount, nonce}
    Frontend->>Contract: 调用 withdrawWithSig(...)
    Contract->>Contract: ecrecover 验证签名 -> 转账 USDC
```
